 ...
 *
 */
// TODO expiring entries? send seqn proactively
/*---------------------------------------------------------------------------*/
#include "contiki.h"
...
struct uc_header {
  ...
  uint8_t pull : 1; // flag to pull data from destination node (sink sets it)
  uint8_t downwards : 1; // signals that the packet should be going down
  uint8_t hops : 5; // statically assert (max_hops <= 0x40)
} __attribute__((packed));
_Static_assert(sizeof(struct uc_header) == 1, "???");
_Static_assert(MAX_HOPS < 0x20, "MAX_HOPS too big, valid range: [0, 0x20)!!!\n");
/*---------------------------------------------------------------------------*/
...
/* Beacon message structure for netowrk recovery */
...
/*---------------------------------------------------------------------------*/
nbr* nbr_list_push(list_t l, const linkaddr_t* new, struct memb* m) {
...
rp_send(struct rp_conn *conn, const linkaddr_t *dest)
{
  ...
  if (packetbuf_hdralloc(expected_sz)) { 
    struct uc_header hdr = { .type = 0, .pull = 0, .downwards = 0, .hops = 0 }; // Prepare the collection header
...
scan_nbrs(struct rp_conn* conn, uint8_t expected_response)
{
  ...
  packetbuf_clear();
  packetbuf_copyfrom(&beacon, sizeof(beacon));
  LOG("INFO: sending recovery beacon: seqn %d metric %d\n",
    conn->beacon_seqn, conn->metric);
  broadcast_send(&conn->bc);
}
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
void
send_pull_update(struct rp_conn* conn, const linkaddr_t* pulled)
{
  /* Parents can request refreshes from supposed childrens if an entry expires 
   * nodes down the tree with updated information may answer the request
   */
  ...
  linkaddr_t links[2];
  linkaddr_copy(links, pulled);
  linkaddr_copy(links + 1, &linkaddr_node_addr);
  if (packetbuf_copyfrom(links, sizeof(links)) != sizeof(links)) {
    LOG("ERROR: topology update too big to fit in packet!\n"); // should not be happening since we have max 36 nodes
    return;
  }

  LOG("INFO: responding to pull request\n");

  if (packetbuf_hdralloc(sizeof(struct uc_header))) { 
    struct uc_header hdr = { .type = 1, .pull = 0, .downwards = 0, .hops = 0 }; // Prepare the collection header
...
rp_msg_handle(struct rp_conn* conn, const linkaddr_t* from, struct uc_header* hdr)
{
  ...
  nbr* tmp = nbr_list_get(nbr_list, &dest);
  if (tmp != NULL) {
    if (clock_time() - tmp->expires > EXPIRED_LIMIT && /* (hdr->downwards || */ conn->is_sink) {
      ...
      */
      ...
      hdr->downwards = 1; // notify that the msg should be going down
      memcpy(packetbuf_dataptr(), hdr, sizeof(struct uc_header));
      unicast_send(&conn->uc, &tmp->next);
      return;
    }
...
uc_recv(struct unicast_conn *c, const linkaddr_t *from) 
{
  ...
  if (!hdr.type) {
    ...
    /* send requested pull info if the sink requested it */
    /*
    if (flag_pull) {
      nbr* tmp = nbr_list_get(nbr_list, &src);
      LOG("INFO: sending information after pull was requested\n");
      if (linkaddr_cmp(&linkaddr_node_addr, &src)) {
        ctimer_set(&conn->unicast_timer, CLOCK_SECOND + T_UPDATE_DELAY, unicast_timer_cb, conn);
      } else if (tmp != NULL && clock_time() - tmp->expires < EXPIRED_LIMIT) {
        send_pull_update(conn, &tmp->dest);
      }
    }
    */
  } else {
