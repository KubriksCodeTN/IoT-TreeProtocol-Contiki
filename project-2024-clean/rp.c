...
/*
  ...
  */
  ...
  ctimer_set(&conn->unicast_timer, delay + .1 * CLOCK_SECOND, unicast_timer_cb, conn);
}
/*---------------------------------------------------------------------------*/
/*                             Unicast Handling                              */
/*---------------------------------------------------------------------------*/
...
t_update_handle(struct rp_conn* conn, const linkaddr_t* from, const struct uc_header* hdr)
{
  ...
  // ! linkaddr_t has size 2 (avoids division) -- this should be true by default defines
 ...
 */
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (tmp == NULL) {
      ...
      // TODO expires
      list_push(nbr_list, tmp);
      linkaddr_copy(&tmp->next, &chain[len - 1]);
    } 
    // TODO always update? Or go for min route?
  ...
  if (packetbuf_copyfrom(chain, (len + 1) << 1) != (len + 1) << 1) {
